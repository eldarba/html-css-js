...
// input.addEventListener('keyup', f4);
// input.addEventListener('keydown', f4);
// input.addEventListener('keypress', f4); // deprecated

// input.addEventListener('focus', f4);
// input.addEventListener('blur', f4);

input.addEventListener('cut', f4);
input.addEventListener('paste', f4);
input.addEventListener('copy', f4);
input.addEventListener('input', f4); // any thing we do on the element

// add select box to html (between input and submit button)
let select = document.querySelector('select');
select.addEventListener('change', f4);
// select.addEventListener('input', f4); // input event works just the same
...
form.addEventListener('submit', f5);

function f5(e) {
    e.preventDefault(); // prevent the default behavior of the element (submit)
    console.log('EVENT TYPE:', e.type, ':' + e.target.tagName);
}
